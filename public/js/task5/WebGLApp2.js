...
export class WebGLApp2 {
    ...
    constructor() {
        ...
        this.circleVelocities = calculateCircleVelocities(100)
        this.circlePos = -1;
...
function calculateCircleVelocities(R, n = 100, speed = 1) {
    let velocities = [];

    for (let i = 0; i < n; i++) {
        let x_i = R * Math.cos((2 * Math.PI * i) / n);
        let y_i = R * Math.sin((2 * Math.PI * i) / n);
        let x_next = R * Math.cos((2 * Math.PI * ((i + 1) % n)) / n);
        let y_next = R * Math.sin((2 * Math.PI * ((i + 1) % n)) / n);
